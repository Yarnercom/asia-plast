...
const News: React.FC<NewsProps> = ({ news }) => {
    ...
    return (
        <section className='container m-auto mb-[44px]'>
            <Toolbar title={'Новости'} link={'/news'} />

            <ViewSwitcher viewMode={viewMode} onChange={setViewMode} />

            <div className={`mt-[50px] gap-4 ${viewMode === 'card' ? 'flex items-center flex-wrap' : 'flex flex-col'}`}>
                {news && news.map((item) => (
                    <div key={item.id} className={`relative ${viewMode === 'card' ? 'w-80' : 'w-full flex items-center p-4'} rounded-xl bg-white bg-clip-border text-gray-700 shadow-md`}>
                        <div
